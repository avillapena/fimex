...
class ProduccionesDetalle extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasMany(UnionInv::className(), ['IdProduccionDetalle' => 'IdProduccionDetalle']);
    }
    ...
    {
        return $this->hasMany(SeriesDetalles::className(), ['IdProduccionDetalle' => 'IdProduccionDetalle'])->with('idSerie');
    }
    ...
    {
        return $this->hasOne(ProduccionesDetalleMaterialVaciado::className(), ['IdProduccionDetalle' => 'IdProduccionDetalle'])->with('idMaterialVaciado');
    }
    ...
    public function getDatos($maquina,$ini,$fin,$area,$subProceso,$turno){
        ...
        if($maquina){
            ...
            $and="AND pr.IdMaquina IN(SELECT IdMaquina FROM v_Maquinas WHERE IdArea = $area AND IdSubProceso = $subProceso)";
        }
        ...
        foreach ($result as &$key) {
            ...
            $anio = date('Y',strtotime($key['Fecha']));
            ...
            $ete = $command->createCommand("
                SELECT DATEPART(isowk, DuxSinc.dbo.v_entrada_plb_detalle.FECHA) AS SEM, SUM(DuxSinc.dbo.v_entrada_plb_detalle.CANTIDAD) as U,
                ...
                WHERE DATEPART(isowk, DuxSinc.dbo.v_entrada_plb_detalle.FECHA) = '$semana' and DATEPART(year, DuxSinc.dbo.v_entrada_plb_detalle.FECHA) = '$anio'
                GROUP BY DATEPART(isowk, DuxSinc.dbo.v_entrada_plb_detalle.FECHA), DuxSinc.dbo.v_entrada_plb_detalle.FECHA")->queryAll();
            ...
            $eteR = $command->createCommand("
                ...
                and datepart(year,DuxSinc.dbo.v_rechaza_plb.FECHA ) = '$anio' 
