...
use frontend\models\calidad\BitacoraCalidad;
use frontend\models\calidad\VMaterialesUsados;
use frontend\models\programacion\VPedidos;
use frontend\models\calidad\EvidenciasCalidad;
use frontend\models\calidad\VBitacoraCalidad;
use frontend\models\calidad\VCalidadResumen;
...
use frontend\models\produccion\TipoProbeta;
...
class ProduccionAceroController extends InventarioController
{
    ...
    public function actionCalidadL(){
        return $this->CapturaProduccion(20,2,51);
    }
    
    public function actionCalidadP(){
        return $this->CapturaProduccion(21,2,48);
    }
    
    public function actionCalidadR(){
        return $this->CapturaProduccion(22,2,51);
    }
    
    public function actionCalidadU(){
        return $this->CapturaProduccion(23,2,49);
    }
    
    public function actionCalidadV(){
        return $this->CapturaProduccion(24,2,47);
    }
    ...
    {
       return $this->CapturaPruebasDestructivas(14,2,2242,52);
    }

    public function actionPruebasmecanicas(){
        return $this->CapturaPruebasDestructivas(14,2,2243,52);
    }
    ...
    */
    ...
    function actionDataProgramaciones(){
        ...
        unset($_REQUEST['Dia']);
        //var_dump($_REQUEST);
        $where = "Semana = ".$_REQUEST['Semana']." AND IdAreaAct IN (5,".$_REQUEST['IdAreaAct'].")";
        $model = VProgramacionesCiclosSemana::find()->where($where)->orderBy('Prioridad ASC')->asArray()->all();
    ...
    public function CapturaProduccion($subProceso, $IdArea, $IdAreaActual = null){
        ...
        switch ($subProceso){
            ...
            case 24: $url = 'CapturaCalidadIV';$title = 'Calidad - Inspeccion Visual'; $subProceso = 14; break;
        }
    ...
    public function actionCountProduccionCalidad(){
        ...
        $model = Producciones::find()
            ...
            ->where("IdArea = $IdArea AND IdSubProceso = $IdSubProceso AND IdCentroTrabajo = $IdCentroTrabajo")
    ...
    public function actionMateriales(){
        $model = VMaterialesCalidad::find()->where([
                ...
                'IdMaterialTipo' =>$_GET['IdMaterialTipo']
    ...
    public function actionGetProdCalidad() {
        if(isset($_GET['IdProduccion'])){
    ...
    public function actionSaveUsados(){         
        //Agrega un registro con el total de los campos almacenados
        $model = new MaterialesUsados();
        $model->load([
            'MaterialesUsados' => [
                'IdProduccion' => $_GET['IdProduccion'],
    ...
    public function getIdBitacoraCalidad(){
        $model = new BitacoraCalidad();

        $model = BitacoraCalidad::find()
            ->select("IdBitacoraCalidad")
            ->where('IdBitacoraCalidad <> 0')
            ->orderBy('IdBitacoraCalidad DESC')
            ->one();

        return $model['IdBitacoraCalidad'];
    }
    public function actionSaveDetalleCalidad(){

        date_default_timezone_set('America/Mexico_City');
        $totalHechas = $_GET['Hechas'];
        ...
        //Obtengo el total de los registros de BitacoraCalidad
        // $model = new BitacoraCalidad();
        // $model = BitacoraCalidad::find()->select('Sum(Cantidad) AS total')->where([
        //         'IdProducto' =>  $_GET['IdProductos'],
        //         'IdProduccionesCalidad' => $IdProduccionesCalidad,
        //         'IdEstatus' => $_GET['IdEstatus']
        //     ])->asArray()->one();

        // $_GET['Hechas'] = $model['total'];
        //END Obtengo el total de los registros de BitacoraCalidad
        
        //Afecta la tabla ProduccionesDetalle

        if( isset($_GET['FechaMoldeo']) ){
        $condFechaMoldeo  =  " AND FechaMoldeo ='".$_GET['FechaMoldeo']."'";
        }else{
        ...
        $model = ProduccionesDetalle::find('IdProduccionDetalle')->where(
            "IdProduccion = ".$_GET['IdProduccion'].
            " AND IdProductos = ".$_GET['IdProductos'].
            " AND IdEstatus = ".$_GET['IdEstatus'].
             $condFechaMoldeo 
        ...
        // if($IdProduccionDetalle == ''){//Es nuevo, hay que agregarlo esto es para editar
        ...
        //Afecta la tabla BitacoraCalidad
        $calidad = new BitacoraCalidad();
        $calidad->load([
            'BitacoraCalidad' => [
                'TPenetracion' => isset($_GET['Penetracion']) ? $_GET['Penetracion'] : '',
                'TRevelado' => isset($_GET['Revelado']) ? $_GET['Revelado'] : '',
                'IdProduccionesDetalles' => $model->IdProduccionDetalle, 
                'IdProducto' => $_GET['IdProductos'],
                'Cantidad' => $_GET['Hechas'],
                'IdUsuario' => Yii::$app->user->getIdentity()->getAttributes()['IdEmpleado'],
                'IdEstatus' => $_GET['IdEstatus'],
                'FechaMoldeo'=>isset($_GET['FechaMoldeo']) ? $_GET['FechaMoldeo'] : '',
            ]
        ]);
        $calidad->save();

        $IdProduccionDetalle = $model->IdProduccionDetalle;
        echo "proddet:".$IdProduccionDetalle;
        if($_GET['IdEstatus'] == 2 || $_GET['IdEstatus'] == 3){
            $IdBitacoraCalidad = $this->getIdBitacoraCalidad();
            for($x=1;$x<=$totalHechas;$x++){
                $evidencias = new EvidenciasCalidad();
                $evidencias->load(['EvidenciasCalidad' => [
                'IdBitacoraCalidad' => $calidad->IdBitacoraCalidad ,
    ...
    public function actionRecibe(){
        ...
        $model = EvidenciasCalidad::findOne($_POST['IdEvidencia']);
        //var_dump($model);
        //exit();
        
        $model->IdMotivo = $_POST['motivos'];
        //$model->IdSerie = $_POST['series'];
        $model->Comentarios = $_POST['observaciones'];
        $model->Imagen = $_FILES[$_POST['nombreImagen']]['name'];
        $model->IdSerie =  $_POST['IdSerie']  !=  "? object:null ?" ?  $_POST['IdSerie'] : 0  ;
        $model->rutaImagen = $path;

        $model->update();
        var_dump($model);
    }
    public function actionLoadCapturados(){
        //$model = ProduccionesDetalle::find()->where($_GET)->asArray()->all();
        $model = VBitacoraCalidad::find()->where($_GET)->asArray()->all();
        foreach($model as &$key){
            // $model1 = Productos::find()->where('IdProducto = '.$key['IdProducto'])->asArray()->one();
            $key['Penetracion'] = isset($key['TPenetracion']) ?  $key['TPenetracion'] : "";
            $key['Revelado'] =  isset($key['TRevelado']) ?  $key['TRevelado'] : "";
            $key['Cantidad'] = $key['Hechas'];
            $key['producto'] = $key['Identificacion'];
            $key['FechaColada'] = $key['FechaMoldeo'];
            // $key['tipo'] = $key['IdEstatus'] == 1 ? "Aceptadas" : ($key['IdEstatus'] == 2 ? "Reparacion" : "Scrap");
            $key['tipo'] = $key['IdEstatus'] ;
        }
        return json_encode($model);
    }

    public function actionCalidadResumen(){
        $model = VCalidadResumen::find()->where($_GET)->asArray()->all();
        return json_encode($model);
    }

    public function actionLoadUsados(){
        $model = VMaterialesUsados::find()->where($_GET)->asArray()->all();
        return json_encode($model);
    }
    public function actionLoadLlevaSerie(){
        $model = Productos::find()->select("LlevaSerie")->where($_GET)->asArray()->one();
        return json_encode($model['LlevaSerie']);
    }
    public function actionLoadEvidenciasCalidad(){
        //var_dump($_GET);
       
                // $IdProduccionDetalle = $this->getIdBitacoraCalidad();
                $IdProduccionDetalle = $_REQUEST['IdProduccionDetalle'];
                $Cantidad = $_REQUEST['Cantidad'];


                 $evidencias = EvidenciasCalidad::find()
                 ->select('count(IdProduccionDetalle) AS total')->where([
                'IdProduccionDetalle' =>  $IdProduccionDetalle,
                ]) ->asArray()->one();
                $evidencias = $evidencias["total"];

                if ( $Cantidad > $evidencias ) 
                    $totalHechas = $Cantidad - $evidencias;
                else 
                    $totalHechas =  $evidencias - $Cantidad ;
                        // echo "tot:".  $totalHechas .  "Can" . $Cantidad ."evid". $evidencias ; exit;
                
                for($x=1;$x<=$totalHechas;$x++){
                            
                        $m = new EvidenciasCalidad();
                        $m->load(['EvidenciasCalidad' => [
                        // 'IdBitacoraCalidad' => $IdBitacoraCalidad ,
                        // 'IdProduccionDetalle' => $_GET['IdProduccionDetalle'],
                        'IdProduccionDetalle' => $IdProduccionDetalle,
                         ]]);
                        $m->save();
            ...
            $model = EvidenciasCalidad::find()->where('IdProduccionDetalle = '.$IdProduccionDetalle)->asArray()->all();
            
            if ( count($model) > 0  ){

                foreach($model AS &$pr){
                    if($pr['rutaImagen'] == '' OR $pr['Imagen'] == '')
                        $pr['imgCompleta'] = 'produccion-acero/preview.png';
                    else    
                        $pr['imgCompleta'] = $pr['rutaImagen'].$pr['Imagen'];
                }
    ...
    public function actionDetalleVaciado(){
        ...
        $model = VProgramacionSemanaAcero::find()->limit($limit)->offset($offset)->where("Anio = '".date('Y',strtotime($_REQUEST['Dia']))."' AND Semana = '".date('W',strtotime($_REQUEST['Dia']))."' AND IdArea = ".$_REQUEST['IdArea']."")->orderBy('IdProgramacion')->asArray()->all();
    ...
    public function actionMaquina(){
        ...
        unset($_REQUEST['IdAreaAct']);
        if($IdAreaAct == 2 && $_REQUEST['IdCentroTrabajo'] == 41 && $_REQUEST['IdSubProceso'] == 9){
            $in = "AND IdSubProceso IN(7,9)";
            $model = VMaquinas::find()->where("IdArea = ".$_REQUEST['IdArea']." $in ")->asArray()->all();
        }else{
            if(isset($_REQUEST)){
    ...
    public function actionTiempos(){
        ...
        if($IdAreaAct == 2 && $_REQUEST['IdSubProceso'] == 9){
            $in = "AND IdSubProceso IN(7,9)";
            //$model = VMaquinas::find()->where("IdArea = ".$_REQUEST['IdArea']." $in ")->asArray()->all();
            $model = \common\models\vistas\VTiemposMuertos::find()->distinct()->select('IdTiempoMuerto,IdMaquina,Identificador,Descripcion,Fecha,Inicio,Fin,Minutos,Observaciones,IdCausa,Causa,IdCausaTipo,ClaveTipo,Tipo,IdArea,IdTurno,IdEmpleado,Orden')->where("IdArea = ".$_REQUEST['IdArea']." 
                                                                           AND IdEmpleado = ".$_REQUEST['IdEmpleado']."
                                                                           AND IdTurno = ".$_REQUEST['IdTurno']." 
                                                                           AND Fecha = '".$_REQUEST['Fecha']."' $in ")->orderBy('Inicio ASC')->asArray()->all();
        }else{
            //$model = VMaquinas::find()->where($_REQUEST)->asArray()->all();
            $model = \common\models\vistas\VTiemposMuertos::find()->distinct()->select('IdTiempoMuerto,IdMaquina,Identificador,Descripcion,Fecha,Inicio,Fin,Minutos,Observaciones,IdCausa,Causa,IdCausaTipo,ClaveTipo,Tipo,IdArea,IdTurno,IdEmpleado,Orden')->where($_REQUEST)->orderBy('Inicio ASC')->asArray()->all();
        }
    ...
    public function actionMostrarSeries2(){
         ...
         extract($_REQUEST); 
            $model = Series::find()
                ->select("
                    Series.Serie,
                    Series.IdSerie,
                    sd.IdProduccionDetalle,
                    pd.IdProduccionDetalle,
                    p.IdProduccion,
                    Series.IdProducto,
                    Series.IdSubProceso,
                    Series.Estatus,
                    ...
                    Series.IdCentroTrabajo
                    ")
                ->leftJoin('dbo.SeriesDetalles AS sd','Series.IdSerie = sd.IdSerie')
                ->leftJoin('dbo.ProduccionesDetalle AS pd','sd.IdProduccionDetalle = pd.IdProduccionDetalle and Series.IdProducto = pd.IdProductos')
                ->leftJoin('dbo.Producciones AS p',' pd.IdProduccion = p.IdProduccion AND Series.IdSubProceso = p.IdSubProceso AND Series.IdCentroTrabajo = p.IdCentroTrabajo')
                ->where(
                'IdProducto='. $IdProducto.
                ' and Series.IdSubProceso='.$IdSubProceso. 
                ' and Series.IdCentroTrabajo = '.$IdCentroTrabajo.
                ' and sd.IdSeriesDetalles is not null'
                    
                )->orderBy('Serie')->asArray()->all();
    ...
    public function actionMostrarSeries(){
        if($_REQUEST['IdSubProceso'] == 14) {
            ...
            $model = Series::find()
                ...
                ' and Series.IdSubProceso='.$IdSubProceso. 
                ' and Series.IdCentroTrabajo = '.$IdCentroTrabajo.
                ' and sd.IdSeriesDetalles is null'
           ...
           //var_dump($producto);exit;
            if($producto->IdAreaAct == 5 && $_REQUEST['IdSubProceso'] < 9 ){
                $where ="IdSubProceso IN(6,7,17)";
                unset($_REQUEST['IdSubProceso']);
                foreach($_REQUEST as $key => $value){
                    $where .= " AND $key = " . (is_string($value) ? "'$value'" : $value);
                }
                //echo $where;exit;
            }else{
                $where = $_REQUEST;
            ...
            $model = Series::find()->where($where)->orderBy('Serie')->asArray()->all();
        }
        return json_encode($model);        
    }
    

    public function actionSaveCalidadSerie(){
      ...
      // var_dump($data);exit;
            ...
            $hechas =  count(SeriesDetalles::find()->where(['IdProduccionDetalle'=>$data['IdProduccionDetalle']])->asArray()->all());
            ...
            // $model = BitacoraCalidad::findOne($data['IdProduccionDetalle']);
            // $model->Cantidad = $hechas;
            // $model->update();
            // var_dump($model);

            // actualizar bitacoracalidad

    }


    public function actionDeleteCalidadSerie(){

        $modelsd = SeriesDetalles::findOne( $_REQUEST);
        $IdProduccionDetalle = $modelsd->IdProduccionDetalle;
        $modelsd->delete();

        $hechas =  count(SeriesDetalles::find()->where(['IdProduccionDetalle'=>$IdProduccionDetalle])->asArray()->all()) ;
            ...
            // $model = BitacoraCalidad::findOne($IdProduccionDetalle);
            // $model->Cantidad = $hechas;
            // $model->update();
            // var_dump($model);
    ...
    public function actionLoadCalidadSerie(){

        $model = SeriesDetalles::find()
        ->select('Series.Serie, SeriesDetalles.IdSeriesDetalles')
        ->leftJoin('ProduccionesDetalle','ProduccionesDetalle.IdProduccionDetalle = SeriesDetalles.IdProduccionDetalle')
        ->leftJoin('Series','Series.IdSerie = SeriesDetalles.IdSerie')
        ->where( ['ProduccionesDetalle.IdProduccionDetalle' =>  $_REQUEST['IdProduccionDetalle'] ] )
        ->asArray()
        ->all(); 
    ...
    public function actionMaterial($IdSubProceso,$IdArea = ''){
        $model = Materiales::find()->where([
                'IdSubProceso' => $IdSubProceso,
                'IdArea'=>$IdArea != '' ? $IdArea :$this->areas->getCurrent()
            ])
            ->with('idMaterialTipo')
            ->asArray()
            ->all();
    ...
    function actionSavePruebas(){
        ...
        if ($data['PruebasDestructivas']['Tipo'] == 1) {
        ...
        if ($data['PruebasDestructivas']['Tipo'] == 2) {
    ...
    public function actionSaveProduccion(){
        ...
        if(isset($data['Producciones']['IdProduccion'])){
            ...
            if(!isset($data['Producciones']['IdTurno'])){
                $data['Producciones']['IdTurno'] = 1;
            }
        ...
        $model1 = null;
        if($data['Producciones']['IdSubProceso'] != 10 && $update == false){
    ...
    public function actionCloseProduccion($IdProduccion){
        ...
        try {
            ...
            $detalles = ProduccionesDetalle::find()->where(['IdProduccion' => $produccion->IdProduccion])->andWhere("Hechas > 0")->with('idProductos')->asArray()->all();
            //var_dump($detalles);
            foreach($detalles as $detalle){
                $produccionDetalle = ProduccionesDetalle::findOne($detalle['IdProduccionDetalle']);
                ...
                if($detalle['idProductos']['LlevaSerie'] == 'Si'){
                    $series = SeriesDetalles::find()->where("IdProduccionDetalle = ".$produccionDetalle->IdProduccionDetalle)->asArray()->all();
                    
                    foreach($series as $serie){
                        $s[] = $serie['IdSerie']*1;
                        $serie = $this->GetSerie(['IdSerie' => $serie['IdSerie']]);

                        if($detalle['idProductos']['PiezasMolde'] > 1){
                            $cavidades = \frontend\models\inventario\SerieCavidad::find()->where(['IdProducto' => $serie['IdProducto']])->asArray()->all();
                            //var_dump($cavidades);
                            foreach($cavidades as $cavidad){
                                $SerieCavidad = $serie['Serie'].$cavidad['Prefijo'].$cavidad['ConsecutivoCavidad'];
                                $SerieCavidad = $this->setSeries([
                                    'IdSeriePadre' => $serie['IdSerie'],
                                    'IdProducto' => $serie['IdProducto'],
                                    'IdSubProceso' => 12,
                                    'Serie' => $SerieCavidad,
                                    'FechaHora' => date('Y-m-d H:i:s'),
                                    'Estatus' => 'B',
                                ]);
                                $s2[] = $SerieCavidad['IdSerie']*1;
                            }
                        }else{
                            $s2[] = $serie['IdSerie']*1;
                        }
                ...
                if($produccionDetalle->Hechas > 0){
                    //$this->generarMovimiento($produccion, $produccionDetalle,$produccion['IdSubProceso'],28,$produccionDetalle->Hechas,'S', $s); //Salida
                    $union = $this->generarMovimiento($produccion, $produccionDetalle,12,42,$produccionDetalle->Hechas * $detalle['idProductos']['PiezasMolde'],'E', $s2); //Entrada
                }
            }
            
            //var_dump($union['IdInventario']);
            $this->actionAfectar($union['IdInventario']);
            $produccion->IdProduccionEstatus = 2;
            if(!$produccion->save()){
                throw new \yii\base\Exception(json_encode($produccion->errors));
            }
            $tran->commit();
    ...
    function actionSaveVaciado(){
        ...
        try {
            ...
            $ProgramacionSemana = $this->getProgramacionSemanal($_REQUEST['IdProgramacion'], date('W',strtotime($_REQUEST['Fecha'])),date('Y',strtotime($_REQUEST['Fecha'])));
            ...
            $produccionDetalle->FechaMoldeo = $_REQUEST['Fecha'];
            ...
            if($Producto->LlevaSerie == 'Si' && $_REQUEST['op'] !== 0){
                $series = SeriesDetalles::find()->where("IdProduccionDetalle = ".$produccionDetalle->IdProduccionDetalle)->asArray()->all();
                //var_dump($series);
                foreach($series as $serie){
                    $s[] = $serie['IdSerie']*1;
            ...
            if($produccionDetalle->Hechas > 0){
                //$this->generarMovimiento($produccion, $produccionDetalle,$produccion['IdSubProceso'],28,$produccionDetalle->Hechas,'S', $s); //Salida
                $this->generarMovimiento($produccion, $produccionDetalle,12,42,$produccionDetalle->Hechas * $Producto->PiezasMolde,'E', $s2); //Entrada
            }
    ...
    function actionSaveDetalleAcero(){
        ...
        try {
            ...
            $ProgramacionSemana = $this->getProgramacionSemanal($produccionDetalle->IdProgramacion, date('W',strtotime($produccion->Fecha)),date('Y',strtotime($produccion->Fecha)));
            $programacionDia = $this->getProgramacionDiaria($ProgramacionSemana->IdProgramacionSemana, $produccion->Fecha, $produccion->IdTurno);
            ...
            foreach ($data['ProduccionesDetalleMoldeo']['IdPartesMoldes'] as $parte) {
                ...
                if ($parte) {
                    ...
                    if(!$ProduccionesCiclos->save()){
                        throw new \yii\base\Exception(json_encode($ProduccionesCiclos->errors));
                    }
                    //var_dump($ProduccionesCiclos);
                    ...
                    if(!$ProduccionesCiclosDetalle->save()){
                        throw new \yii\base\Exception(json_encode($ProduccionesCiclosDetalle->errors));
                    }
            ...
            if(!$produccionDetalle->update()){
                throw new \yii\base\Exception(json_encode($produccionDetalle->errors));
            }
            ...
            if($data['ProduccionesDetalleMoldeo']['LlevaSerie'] == 'Si' && $IdParteMolde == $producto->IdParteMolde){
                ...
                if( isset($ProduccionesCiclosDetalle)){
                    foreach ( $ProduccionesCiclosDetalle_arr as $ciclosDetalle){
                        ...
                        if(!$SeriesDetalleProdCiclosDetalle->save()){
                            throw new \yii\base\Exception(json_encode($SeriesDetalleProdCiclosDetalle->errors));
                        }
    ...
    function updateConfSeries($IdConfiguracionSerie){
        ...
        if(!$configuracion->save()){
            throw new \yii\base\Exception(json_encode($configuracion->errors));
        }
    }
    
    function setSeriesDetalle($data){
        $data['IdProduccionDetalle'] *=1;
        $data['IdSerie'] *=1;
        $serie = SeriesDetalles::find()->where([
            'IdProduccionDetalle' => $data['IdProduccionDetalle'],
            'IdSerie' => $data['IdSerie']
        ])->one();
        
        if(!isset($serie->IdSeriesDetalles)){
            $serie = new SeriesDetalles();
            $serie->load(['SeriesDetalles' => $data]);
            if(!$serie->save()){
                throw new \yii\base\Exception(json_encode($serie->errors));
            }
        }
        return $serie;
    }
    ...
    function SaveLance($data,$produccion){
        ...
        $model = VLances::find()->where([
            'IdArea' => $this->areas->getCurrent(),
            'Anio' => date('Y',strtotime($data['Fecha']))
    ...
    function actionSaveConsumo(){
        if(isset($_REQUEST['$$hashKey'])){
            unset($_REQUEST['$$hashKey']);
        }
        
        if(isset($_REQUEST['idMaterial'])){
            unset($_REQUEST['idMaterial']);
        }
        ...
        $model = MaterialesVaciado::find()->where($_REQUEST)->with('idMaterial')->asArray()->one();
        return json_encode(
            $model
        );
    }
...
// C:\fakepath\  si encuentra fakepath en el nombre de la imagen lo corta
        
        if(strpos( $data['tratamientos']['Imagen'], "fakepath") > 0)
             $data['tratamientos']['Imagen'] =   substr($data['tratamientos']['Imagen'],12);
        ...
        //echo "tratamientos" ;//var_dump($tratamientos);
    ...
    public function actionBusquedatt(){
    ...
    public function actionProductosprogramadostt(){
        $where = "";
    ...
    public function actionSaveDetallett(){
        ...
        $datos = [
            'FechaMoldeo' => isset($_REQUEST['FechaMoldeo']) ?  $_REQUEST['FechaMoldeo'] : 'sin',
        ...
        $model = ProduccionesDetalle::find()->where(['IdProduccionDetalle' =>$datos['IdProduccionDetalle'] ] )->one();
    ...
    public function actionSaveseriestt(){
        ...
        $model = SeriesDetalles::find()->where(['IdSeriesDetalles' =>$datos['IdSeriesDetalles'] ] )->one();
    ...
    public function actionLoadprobetavaciado(){  
    ...
    public function actionDeleteProducciones(){
            ...
            $detalle = ProduccionesDetalle::find()->where("IdProduccion = ".$_REQUEST['IdProduccion']." ")->asArray()->all();
    ...
    public function actionSaveprobetadetallevaciado(){   
        // 'Cantidad' => string '2' (length=1)
        // 'IdLance' => string '9330' (length=4)
        // 'Tipo' => string 'Charpy' (length=6)
        unset($_GET['active']);
        unset($_GET['$$hashKey']);
        $idlance = isset ($_GET['$IdLance']) ? $_GET['$IdLance'] : 0;

        //var_dump( $_GET );

        $model = Probetas::find()->where(['IdLance' => $idlance ])->one();
        if(is_null($model))
            $model = new Probetas();
        $model->load(['Probetas' => $_GET]); 
        $model->save();
    //var_dump($model);
    }
    ...
    public function actionDeleteprobetasdetallevaciado(){
        //var_dump( $_GET );
        $Probeta  = Probetas::findOne( $_REQUEST['IdProbeta'])->delete();
        //var_dump($Probeta);
    }


    public function actionTipoProbeta(){
        $model = TipoProbeta::find()->asArray()->all();
        return json_encode ($model);
    }
    ...
    public function actionGetProducto(){
        $model =  Productos::find()->where($_REQUEST)->asArray()->all();
        return json_encode($model);
    }
    ...
    public function actionSaveEstatusMonitoreo(){
        ...
        if ($tipomonitoreo != null) {   
            ...
            switch (true) {
                case ($tipomonitoreo['Descripcion'] == 'Liberado'):
                    $Estatus = "A7DA4E"; break;
                case ($tipomonitoreo['Descripcion'] == 'Proceso'):
                    $Estatus = 'FB8C3C'; break;
                case ($tipomonitoreo['Descripcion'] == 'Desarrollo'):
                    $Estatus = 'ED1C24'; break;
                case ($tipomonitoreo['Descripcion'] == 'Prueba'):
                    $Estatus = '0072BC'; break;
            }

            if ($tipomonitoreo['IdTipoMonitoreo'] == 2) {
                $model = new Productos();
                $model = $model::findOne($IdProducto);
                $model->ColorEstatus = $Estatus;
                $model->update();
                var_dump($model);
            }
        }
    ...
    // EDICION de moldeo y cerrado
    public function actionActivaUndo(){
    ...
    public function actionDeleteCiclosDetalle(){
        ...
        if ($estatus == 'Repo' || $estatus == 'Rechazo'){
            $cilcosmolde = 1;
        }else{
    ...
    function getProgramacionSemanal($IdProgramacion, $Semana, $Anio){
    ...
    function actualizaProgramacionDiaria($IdProgramacion, $Fecha, $IdProgramacionDia){
        ...
        if(!$IdProgramacionDia->save()){
            throw new \yii\base\Exception(json_encode($IdProgramacionDia));
        }
        //var_dump($IdProgramacionDia);
        
        $command = \Yii::$app->db;
        $command->createCommand("EXECUTE p_ActualizaTotalesSemana ".$IdProgramacionDia->IdProgramacionSemana)->execute();
    }
    
    function actionActualizaSemana($anio,$semana,$IdArea){
        $model = \frontend\models\produccion\VProduccion2::find()->distinct()->select('Fecha,IdProgramacion,IdTurno')->where([
            'Anio' => $anio,
            'Semana' => $semana,
            'IdArea' => $IdArea
        ])->asArray()->all();
        ...
        foreach($model as $mod){
            //var_dump($mod);
            $ProgramacionSemana = $this->getProgramacionSemanal($mod['IdProgramacion'], date('W',strtotime($mod['Fecha'])),date('Y',strtotime($mod['Fecha'])));
            $programacionDia = $this->getProgramacionDiaria($ProgramacionSemana->IdProgramacionSemana, $mod['Fecha'], $mod['IdTurno']);
            
            //var_dump($programacionDia);

            $this->actualizaProgramacionDiaria($mod['IdProgramacion'],$mod['Fecha'],$programacionDia);
        }
    }
    ...
    function actionNow(){
        $Fecha1 = date('Y-m-d');
        //$Fecha1 = '2016-03-17';
        $Fecha2 = date('Y-m-d',strtotime($_REQUEST['Fecha']));
        
        if((strtotime($Fecha2) >= strtotime($Fecha1)) || Yii::$app->user->identity->role == 1)
            return true;
        else
            return false;
    }
