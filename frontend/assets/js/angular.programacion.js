...
app.controller('Programacion', function($scope, $filter, ngTableParams, $http, $timeout){
    ...
    $scope.validarCierre = function(programacion){
        if(programacion.Cerrar == 0){
            return 0;
        }
        
        console.log(programacion.SaldoCantidad,parseInt(programacion.Programadas));
        if(programacion.SaldoCantidad <= 0 && parseInt(programacion.Programadas) <= 0){
            return 1;
        }

        alert('No se puede cerrar esta linea por que aun tienen balance o tiene programacion en semanas posteriores');
        return 0;
    };
    ...
    $scope.cerrarPedido = function(){
        ...
        $scope.programaciones.forEach(function(value,key){
            if(value.Cerrar == 1){
                $scope.selectedCerrado.push(value.IdProgramacion);
            }
        });
    ...
    $scope.loadMarcas = function(){
        return $http.get('marcas',{params:{IdArea:$scope.IdArea}}).success(function(data){
            $scope.clientes = [];
            $scope.clientes = data;
        });
    };
    ...
    $scope.loadResumenDiario = function(){
        if ($scope.IdArea === 2) {
            return $http.get('resumen-diario-acero',{params:{IdProceso:$scope.IdProceso,semana:$scope.semanaActual}})
            .success(function(data){
                $scope.resumenes = data;
    ...
    $scope.loadResumenSemanalAcero = function(){
        return $http.get('resumen-semanal-acero',{params:{IdProceso:$scope.IdProceso,IdArea:$scope.IdArea, semana:$scope.semanaActual}}).success(function(data){
            $scope.resumenes = data;
        }).error(function(data, status, headers, config){
