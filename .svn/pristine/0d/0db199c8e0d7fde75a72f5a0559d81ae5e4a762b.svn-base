    ...
    <div class="panel panel-default">
        ...
        <table ng-table fixed-table-headers="scrollable-area" class="table table-striped table-bordered table-hover ">
            ...
            <tbody style="font-size: 10pt">
                ...
                <?php for($x=1;$x<=6;$x++):?>
                    <?php $class = $x % 2 != 0 ?'par' : 'impar'; ?>
                    <th class="cap"><input ng-disabled="true" class="filter" style="width: 33px; font-size: 9pt;" ng-model-options="{updateOn: 'blur'}" onkeypress="return justNumbers(event)" ng-change="saveProgramacionDiaria(programacion,<?=$x?>);" ng-model="programacion.Programadas<?=$x?>"></th>
                    <?php if($IdSubProceso == 12): ?>
                    <th class="<?=$class?>"><select ng-disabled="true" ng-model-options="{updateOn: 'blur'}" ng-change="saveProgramacionDiaria(<?=$x?>);" ng-model="programacion.IdCentroTrabajo<?=$x?>">
