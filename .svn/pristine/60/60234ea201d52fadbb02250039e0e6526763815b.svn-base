...
class Programacion extends \yii\db\ActiveRecord
{
    ...
    {   
        ...
        $command = \Yii::$app->db;
        //echo "SELECT * FROM f_GetProgramacionesDiaria". ($IdProceso == 2 ? "Limpieza" : "") ."($IdArea,$IdProceso,$params,$turno) Order By Posicion ASC, Producto ASC";
        $result =$command->createCommand("SELECT * FROM f_GetProgramacionesDiaria". ($IdProceso == 2 ? "Limpieza" : "") ."($IdArea,$IdProceso,$params,$turno) Order By Posicion ASC, Producto ASC")->queryAll();
