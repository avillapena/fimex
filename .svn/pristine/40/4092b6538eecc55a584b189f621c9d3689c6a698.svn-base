...
use frontend\models\programacion\Embarques;
use frontend\models\programacion\ProgramacionesAlma;
use frontend\models\programacion\ProgramacionesAlmaSemana;
use frontend\models\programacion\VPedidosEmbarquesAceros;
use frontend\models\programacion\VPedidosEmbarquesBronces;
...
class ProgramacionController extends Controller
{
    ...
    public function actionMarcas(){
        $this->layout = 'JSON';
        $model = new Pedidos();
        $dataProvider = $model->getMarcas($_REQUEST['IdArea']);
    ...
    public function actionEstatusProducto($IdProducto){
            ...
            foreach ($result as &$value) {
                switch(true) {
                    case (trim($value['Concepto']) == 'Desarrollo') && ($desarrollo == 1):
                        $Estatus['EstatusDesarrollo'] = $value['EstatusDesarrollo'];
                        $desarrollo = 2;
                        break;
                    case (trim($value['Concepto']) == 'Almas') && ($almas == 1):
                        $Estatus['EstatusAlmas'] = $value['EstatusAlmas'];
                        $almas = 2;
                        break;
                    case (trim($value['Concepto']) == 'Modelo') && ($modelos == 1):
                        $Estatus['EstatusModelos'] = $value['EstatusModelo'];
                        $modelos = 2;
                        break;
                    case (trim($value['Concepto']) == 'Cajas Almas') && ($cajas == 1):
            ...
            switch (true) {
                case ($Estatus['EstatusDesarrollo'] == 'Liberado'):
                    $Estatus['ColorProducto'] = "#A7DA4E";
                    break;
                case ($Estatus['EstatusDesarrollo'] == 'Proceso'):
                    $Estatus['ColorProducto'] = '#FB8C3C';
                    break;
                case ($Estatus['EstatusDesarrollo'] == 'Desarrollo'):
                    $Estatus['ColorProducto'] = '#ED1C24';
                    break;
                case ($Estatus['EstatusDesarrollo'] == 'Prueba'):
                    $Estatus['ColorProducto'] = '#0072BC';
                    break;
            }
    ...
    public function actionDataSemanal(){
        ...
        foreach($semanas as $semana){
            $this->actionActualizaSemana($semana['year'], $semana['week'], $IdArea);
        }
...
//                'footer'=>$dataResumen[0],
    ...
    public function actionSaveSemanal()
    {
        ...
        if($model->setProgramacionSemanal($datosSemana1)){
            ...
            $this->SetBitacora("Programacion Actualizada Producto: ".$dat['Producto']." Año: ".$dat['Anio'].", Semana: ".$dat['Semana'],"ProgramacionSemanal","Prioridad",$dat['Prioridad'],"");
        }
    }
    ...
    function actionEmbarquesAceros(){
        return $this->embarques(2);
    }
    
    function actionEmbarquesJuntaAceros(){
        return $this->embarquesReporte(2,true);
    }
    
    function actionEmbarquesInformeAceros(){
        return $this->embarquesReporte(2);
    }
    
    function actionEmbarquesJuntaBronces(){
        return $this->embarquesReporte(3,true);
    }
    
    function actionEmbarquesInformeBronces(){
        return $this->embarquesReporte(3);
    }
    
    function actionEmbarquesBronces(){
        return $this->embarques(3);
    }
    
    function embarques($IdArea){
        return $this->render('Embarques',['IdArea' => $IdArea]);
    }
    
    function embarquesReporte($IdArea,$Junta = 'false'){
        return $this->render('EmbarquesReporte',[
            'IdArea' => $IdArea,
            'Junta' => $Junta
        ]);
    }

    function actionPedidosClientes(){
        return $this->PedidosClientes(2);
    }
    
    function actionPedidosClientesBronces(){
        return $this->PedidosClientes(3);
    }
    
    function PedidosClientes($IdArea){
        return $this->render('PedidosClientes',[
            'IdArea' => $IdArea,
            'role' => Yii::$app->user->identity->role
        ]);
    }

    function actionAnalisisPedidos(){
        return $this->AnalisisPedidos(2);
    }
    
    function actionAnalisisPedidosBronces(){
        return $this->AnalisisPedidos(3);
    }
    
    function AnalisisPedidos($IdArea){
        return $this->render('AnalisisPedidos',['IdArea' => $IdArea]);
    }

    function actionSaveEmbarque(){
        if(isset($_REQUEST['IdEmbarque'])){
            $model = Embarques::findOne($_REQUEST['IdEmbarque']);
            unset($_REQUEST['IdEmbarque']);
        }else{
            $model = new Embarques();
        }

        $model->load(['Embarques' => $_REQUEST]);
        $model->save();
        var_dump($model);
    ...
    function actionGetPedidos($IdArea){
        switch($IdArea){
            case 1:
                $class = '';
                break;
            case 2:
                $model = VPedidosEmbarquesAceros::find()->where('Balance > 0')->asArray()->all();
                break;
            case 3:
                $model = VPedidosEmbarquesBronces::find()->where('Balance > 0')->asArray()->all();
                break;
        }
    ...
    function actionGetClientes($IdArea){
        switch($IdArea){
            case 1:
                $class = '';
                break;
            case 2:
                $model = VPedidosEmbarquesAceros::find()->distinct()->select('NOMBRE')->asArray()->all();
                break;
            case 3:
                $model = VPedidosEmbarquesBronces::find()->distinct()->select('NOMBRE')->orderBy('NOMBRE')->asArray()->all();
                break;
        }
        //var_dump($model);exit;
        return json_encode($model);
    ...
    function actionUpdateEmbarque(){
        $model = Embarques::find()->where($_REQUEST)->one();
        $model->load(['Embarques' => $_REQUEST]);
        $model->update();
    }
    
    function actionDeleteEmbarque($IdEmbarque){
        $model = Embarques::findOne($IdEmbarque)->delete();
    }
    ...
    function actionActualizaSemana($anio,$semana,$IdArea){
        ...
        foreach($model as $mod){
            $ProgramacionSemana = $this->getProgramacionSemanal($mod['IdProgramacion'], date('W',strtotime($mod['Fecha'])));
            if($ProgramacionSemana !== null){
    ...
    function actualizaProgramacionDiaria($IdProgramacion, $Fecha, $IdProgramacionDia){
        $model = \frontend\models\produccion\VProduccion2::find()
