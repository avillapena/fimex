<div class="panel panel-primary">
    ...
    <div class="panel-heading">Captura de produccion --Productos Cargados: {{detalles.length}}--</div>
    <div id="detalle" class="scrollable">
        <table ng-table class="table table-condensed table-striped table-bordered">
            ...
            <tbody>
                <tr ng-class="{'info': indexDetalle == $index}"  ng-repeat="detalle in detalles" ng-if="(produccion.IdProduccionEstatus == 1 ? true : (detalle.Hechas > 0 || detalle.Rechazadas > 0 ? true : false) || detalle.Semana == produccion.Semana)">
                    ...
                    <th>
                        <input ng-disabled="detalle.LlevaSerie == 'Si'" ng-if="isNow" type="text" name="Vaciadas" id="Vaciadas" ng-model="detalle.Hechas" ng-value="{{detalle.Hechas}}" >
                        <span ng-if="!isNow">{{detalle.Hechas}}</span>
                    </th>  
                    <th>
                        <!-- encaso de agregar cerrado a moldeo especial quitar la validacion "detalle.IdAreaAct == 3 ? 17 : del codigo" -->
                        <button ng-if="isNow" class="btn btn-primary" ng-show="detalle.LlevaSerie == 'Si'" ng-click="MostrarSeries(detalle.IdProducto,detalle.IdAreaAct == 3 ? 17 : 9); ModelMoldeo(detalle.Producto, detalle.IdProducto,$index,8,1,1);">Series</button>
                        <button ng-if="isNow" class="btn btn-success btn-xs" ng-show="detalle.LlevaSerie != 'Si'" ng-click="saveDetalleVaciado($index,0,1)"><span class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></span></button>
                    </th>
                  
                    <th>
                        <input ng-disabled="detalle.LlevaSerie == 'Si'" ng-if="isNow" type="text" name="RechazosV" id="RechazosV" ng-model="detalle.Rechazadas" ng-value="{{detalle.Rechazadas}}" >
                        <span ng-if="!isNow">{{detalle.Rechazadas}}</span>
                    </th>
                    <th>
                        <!-- encaso de agregar cerrado a moldeo especial quitar la validacion "detalle.IdAreaAct == 3 ? 17 : del codigo" -->
                        <button ng-if="isNow" class="btn btn-primary" ng-show="detalle.LlevaSerie == 'Si'" ng-click="MostrarSeries(detalle.IdProducto,detalle.IdAreaAct == 3 ? 17 : 9); ModelMoldeo(detalle.Producto, detalle.IdProducto,$index,8,0,0);">Series</button>
                        <button ng-if="isNow" class="btn btn-success btn-xs" ng-show="detalle.LlevaSerie != 'Si'" ng-click="saveDetalleVaciado($index,0,0)"><span class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></span></button>
                    </th>  
