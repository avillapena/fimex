    ...
    <table style="width:100%" id="ff" class="table table-striped">
        ...
        <tbody>
            ...
            <?php foreach($semanas as $semana): ?>
                <th><?= $detail[$semana['year'].'-S'.($semana['week']*1)] == null ? 0 : $detail[$semana['year'].'-S'.($semana['week']*1)] ?> </th>
                <?php 
                    $sum2[$semana['year'].'-S'.($semana['week']*1)] = isset($sum2[$semana['year'].'-S'.($semana['week']*1)]) ? $sum2[$semana['year'].'-S'.($semana['week']*1)] : 0;
                    $sum2[$semana['year'].'-S'.($semana['week']*1)] += $detail[$semana['year'].'-S'.($semana['week']*1)];
                ?>
            <?php endforeach; ?>
                <th><?= $detail['PesoTot'] ?> </th>
            </tr>
        ...
        <tfoot>
            <tr>
                ...
                <?php $total= 0; ?>
                <?php foreach($semanas as $semana): ?>
                    <?php $total += $sum2[$semana['year'].'-S'.($semana['week']*1)]?>
                    <th><?= $sum2[$semana['year'].'-S'.($semana['week']*1)]?></th>
                <?php endforeach; ?>
                <th><?=$total?></th>
        </tfoot>
    </table>
