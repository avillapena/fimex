 ...
 */
app.controller('ProgramacionLimpieza', function($scope, $filter, ngTableParams, $http, $timeout){
    ...
    $scope.loadResumenDiario = function(){
        return $http.get('resumen-diario',{params:{IdProceso:$scope.IdProceso,semana:$scope.semanaActual}})
        .success(function(data){
    ...
    $scope.loadResumenCelda = function(){
        return $http.get('resumen-celda',{params:{
                semana:$scope.semanaActual,
            }}).success(function(data){
            $scope.resumenesCelda = data;
        }).error(
            function(data, status, headers, config){
            }
        );
    ...
    $scope.saveProgramacionDiaria = function(row,dia){
        ...
        total = 0;
        for(x=1;x<7;x++){
            if(!isNaN(parseInt(row['Programadas'+x]))){
                total += parseInt(row['Programadas'+x]);
            }
        }

        if(total <= row['Programadas']){
            return $http.get('save-diario',{params:{
                ...
                IdTurno:row.IdTurno,
                IdCentroTrabajo:row['IdCentroTrabajo'+dia],
            ...
            $scope.loadProgramacionDiaria(row.Tarimas == 'undefined' ? false : true);
            alert("No se puede programar mas de lo asignado a la semana");
            $scope.login(false);
        }
    };
