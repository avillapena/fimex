    ...
    <div class="row">
        ...
        <div class="col-md-4">
            ...
            <select aria-describedby="Maquinas" class="form-control input-sm" ng-model="mtto.IdMaquina" required>
                <option ng-show="{{maquina.Descripcion == 'Fusion' ? true : false }}" value="{{maquina.IdMaquina}}" ng-repeat="maquina in maquinas">{{maquina.ClaveMaquina}} - {{maquina.Maquina}}</option>
            </select>
        </div>
        <div class="col-md-3">
    ...
    <table class="table table-striped table-responsive table-bordered"
        ...
        <tbody>
            <tr ng-repeat="dato in datos | orderBy:arr | filter:{
                    ...
                    <td >{{dato.Fecha}}</td>
                    <td>{{dato.ClaveMaquina}} - {{dato.Maquina}}</td>
